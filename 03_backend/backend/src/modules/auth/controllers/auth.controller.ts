...
export class AuthController {
  ...
  public async login(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!data) return res.status(401).json('Invalid Credentials');
  ...
  public async register(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!data) return res.status(500).json('Error Registering User');
  ...
  public async updateById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      await this.service.updateById(id, plainToInstance(UserEntity, dto));
      ...
      token = await this.service.login(data?.role.id!, data?.id!);
