...
export class UserController {
  ...
  public async createNew(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const exists: UserEntity | null = await this.service.getByEmail(
        dto.email,
      );
      ...
      if (!data) return res.status(500).json('Error Creating User');

      const newUserData: UserEntity | null = await this.service.getById(
        data.id,
      );
  ...
  public async updateById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      // Si no se encuentra el usuario, devuelve un error 404.
      if (!toUpdate) return res.status(404).json('User Not Found');
      ...
      if (!updatedData) return res.status(500).json('Error Updating User');
  ...
  public async deleteById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!deleteResult) return res.status(500).json('Error Deleting User');
